...
export function crudRouter(definicion) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (termino && buscables.length > 0) {
        const partes = buscables.map((campo) => `${origenPrefijo}${columnaSegura(campo)} LIKE ?`);
        condiciones.push(`(${partes.join(" OR ")})`);
        buscables.forEach(() => valores.push(`%${termino}%`));
      }

      filtros.forEach((campo) => {
        const valor = req.query[campo];
        if (valor !== undefined && valor !== "" && valor !== "todos") {
          condiciones.push(`${origenPrefijo}${columnaSegura(campo)} = ?`);
          valores.push(valor);
        }
      });
