  ...
  asyncHandler(async (req, res) => {
    ...
    const filas = await query(
      `SELECT m.id_modulo, m.codigo, m.nombre, m.ubicacion, m.estado,
              ...
              COALESCE(d.porcentaje_defectos, 0)  AS porcentaje_defectos,
              ...
              -- Los minutos, que son la unidad en la que se mide todo:
              -- los puestos, los ganados y los que se fueron por causa.
              COALESCE(d.minutos_disponibles, 0)  AS minutos_disponibles,
              COALESCE(d.minutos_ganados, 0)      AS minutos_ganados,
              COALESCE(d.minutos_jornada, 0)      AS minutos_jornada,
              COALESCE(d.minutos_perdidos, 0)     AS minutos_perdidos,
              COALESCE(d.minutos_perdidos_persona, 0) AS minutos_perdidos_persona,
              COALESCE(d.minutos_maquina, 0)      AS minutos_maquina,
              COALESCE(d.minutos_calidad, 0)      AS minutos_calidad,
              COALESCE(d.minutos_montaje, 0)      AS minutos_montaje,
              COALESCE(d.minutos_otras, 0)        AS minutos_otras,
              d.sam_observado,
              o.numero_orden, o.codigo_referencia, o.nombre_referencia,
              o.nombre_cliente, o.valor_maquila_unidad,
              o.sam_pactado, o.porcentaje_avance,
              -- Cuanto dura el dia de planta en esa fecha. Reemplaza al
              -- modulos.horas_jornada que se digitaba: el horario es de
              -- la planta y sale de las franjas, no de cada modulo.
              COALESCE(hj.minutos_totales, 0)     AS minutos_horario,
              COALESCE(hj.horas_totales, 0)       AS horas_horario
       FROM modulos m
       ...
       LEFT JOIN jornada_dia jd ON jd.dia_semana = WEEKDAY(?) + 1
       LEFT JOIN vw_horario_jornada hj ON hj.id_jornada = jd.id_jornada
       LEFT JOIN (
         ...
         JOIN (
           ...
           WHERE estado = 'EN_PROCESO' AND id_modulo IS NOT NULL
           GROUP BY id_modulo
...
indicadoresRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // `minutos_perdidos` ahora son minutos-persona MEDIDOS: la digitadora
...
indicadoresRouter.get(
  ...
  asyncHandler(async (req, res) => {
    res.json({
      datos: await query(
        `SELECT codigo_referencia, nombre_referencia, nombre_cliente,
         ...
         GROUP BY codigo_referencia, nombre_referencia, nombre_cliente
