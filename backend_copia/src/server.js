...
/**
 * Fichas tecnicas subidas con el lote.
 *
 * Son archivos estaticos, no datos: se sirven directo desde disco en vez
 * de pasar por la API. `CARPETA_SUBIDAS` la crea multer la primera vez
 * que alguien sube una ficha, de ahi el `fallthrough` en false: si el
 * archivo no esta, responde 404 y no se cuela al router de la API.
 */
app.use(
  "/uploads/fichas",
  express.static(CARPETA_SUBIDAS, { fallthrough: false, maxAge: "1d" }),
);
